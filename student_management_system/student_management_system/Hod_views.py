...
from app.models import (
    ...
    Student_Feedback,Student_Leave
)
...
def ADD_STUDENT(request):
    ...
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            messages.success(
                request,
                user.first_name + " " + user.last_name + " is successfully added.",
            )
            return redirect("add_student")
...
def DELETE_STUDENT(request, admin):
    ...
    student.delete()
    messages.success(request, "Records Are Sucessfully Deleted!")
    return redirect("view_student")
...
def UPDATE_COURSE(request):
    if request.method == "POST":
        ...
        course.name = name
        course.save()
        messages.success(request, "Course Sucessfully Updated!!")
...
def DELETE_COURSE(request, id):
    course = Course.objects.get(id=id)
    course.delete()
    messages.success(request, "Course Sucessfully Deleted!")
    return redirect("view_course")
...
def STAFF_SEND_NOTIFICATION(request):
    staff = Staff.objects.all()
    staff_notification = Staff_Notification.objects.all()
    context = {"staff": staff, "staff_notification": staff_notification}
...
def SAVE_STAFF_NOTIFICATION(request):
    if request.method == "POST":
        ...
        staff = Staff.objects.get(admin=staff_id)
        staff_notifications = Staff_Notification(staff_id=staff, message=message)
...
def APPROVE_STAFF_LEAVE(request, id):
    staff_leave = Staff_Leave.objects.get(id=id)
    staff_leave.status = 1
    staff_leave.save()
    return redirect("hod_staff_leave")
